...
describe('pivot', function () {
  ...
  it('should mutate the array by placing values on either side of the pivot', function () {
    ...
    expect(arr3.slice(0, 1).sort(strLength)).toEqual(
      ["Blue"],
      "After calling pivot with the strLength comparator, 'Blue' should be to the left of 'LilBub' in arr3."
    );
    expect(arr3.slice(1).sort(strLength)).toEqual(
      ["LilBub", "Grumpy", "Garfield", "Heathcliff"],
      "After calling pivot with the strLength comparator, 'Grumpy', 'Garfield', and 'Heathcliff' should be to the right of 'LilBub' in arr3."
    );
  });
});
...
describe('quicksort', function () {
  ...
  it('should sort by the comparator if a comparator is provided', function () {
    ...
    expect(quickSort(kitties, strComp)).toEqual(
      ...
      'quickSort(["LilBub", "Garfield", "Heathcliff", "Blue", "Grumpy"], strComp) should equal ["Blue", "Garfield", "Grumpy", "Heathcliff", "LilBub"]'
    );
    expect(quickSort(moarKittyData, oldestToYoungest)).toEqual([{
      ...
      "quickSort(moarKittyData, oldestToYoungest) should sort the kitties from oldest to youngest.")
  });
});
